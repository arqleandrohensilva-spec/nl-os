...
import { ChevronRight, ChevronLeft, Check, ClipboardCheck, AlertCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { motion, AnimatePresence } from 'framer-motion';

type QuestionType = 'choice' | 'text' | 'multi' | 'investment' | 'decisao';
    ...
    <div className=\"min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6\">
      <div className=\"max-w-md w-full bg-[#141414] border border-white/10 p-10 text-center space-y-6\">
        ...
        <p className=\"text-white/60 text-sm leading-relaxed\">
          O link utilizado parece estar incorreto ou o projeto não está mais disponível.
        </p>
        <Button 
          variant=\"outline\" 
          className=\"w-full border-white/10 text-white/60 uppercase tracking-widest text-[10px] h-12\"
          onClick={() => navigate('/')}
        >
          Voltar para Início
        </Button>
  ...
  if (isFinished) return (
    <div className=\"min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6 font-['Courier_New'] relative overflow-hidden\">
      <div className=\"absolute inset-0 opacity-20\">
        <img src={BACKGROUND_IMAGES[0]} alt=\"Bg\" className=\"w-full h-full object-cover\" />
      </div>
      ...
      >
        <div className=\"space-y-4\">
          <p className=\"text-bronze text-[10px] font-bold tracking-[0.5em] uppercase\">NL ARQUITETOS</p>
          <div className=\"w-12 h-[1px] bg-bronze/30 mx-auto\" />
        </div>
        
        <div className=\"space-y-6\">
          <h1 className=\"text-white font-['Georgia'] text-4xl md:text-5xl italic\">Briefing recebido.</h1>
          <p className=\"text-white/60 text-xs md:text-sm uppercase tracking-widest leading-loose max-w-md mx-auto\">
            A NL Arquitetos revisará suas respostas antes da reunião.
          </p>
        </div>

        <div className=\"pt-12 border-t border-white/10\">
          <p className=\"text-white/20 text-[9px] uppercase tracking-[0.8em]\">A arquitetura como decisão.</p>
        </div>
      </motion.div>
    </div>
  );
    ...
    <div className=\"min-h-screen bg-[#0a0a0a] text-white flex flex-col font-['Courier_New'] relative overflow-hidden\">
      ...
      <header className=\"relative z-10 p-8 flex justify-between items-center\">
        <div className=\"space-y-1\">
          <p className=\"text-bronze text-[10px] font-bold tracking-[0.5em]\">NL ARQUITETOS</p>
            ...
            {step === -1 ? (
              ...
              >
                <div className=\"space-y-6\">
                  ...
                  <p className=\"text-white/60 text-xs md:text-sm uppercase tracking-widest leading-relaxed max-w-lg\">
                ...
                <div className=\"pt-8\">
                  <Button 
                    onClick={() => setStep(0)}
                    className=\"bg-bronze hover:bg-bronze/80 text-white rounded-none h-16 px-12 text-xs font-bold tracking-[0.3em] uppercase transition-all\"
                  >
                    Começar Briefing
                  </Button>
                </div>
              ...
              >
                <div className=\"space-y-4\">
                  <span className=\"text-bronze font-bold text-[10px] tracking-[0.4em] uppercase\">Questão {step + 1} de {questions.length}</span>
                  <h2 className=\"text-3xl md:text-5xl font-['Georgia'] italic leading-tight\">
                    {currentQuestion?.title}
                  </h2>
                  {currentQuestion?.subtitle && (
                    <p className=\"text-white/50 text-[11px] uppercase tracking-widest leading-relaxed\">
                      {currentQuestion.subtitle}
                    </p>
                  )}
                </div>
                ...
                <div className=\"py-6\">
                    ...
                    <div className=\"grid gap-3 max-w-xl\">
                      {currentQuestion.options?.map(option => (
                        <button
                          ...
                          className={cn(
                            \"w-full text-left p-6 border transition-all duration-300 flex justify-between items-center group\",
                            answers[currentQuestion.id] === option
                              ? \"bg-white text-black border-white\"
                              : \"bg-white/5 border-white/10 hover:border-white/30 text-white/80\"
                          )}
                        >
                          <span className=\"text-[11px] uppercase tracking-widest font-bold\">{option}</span>
                      ...
                      {currentQuestion.options?.map(option => {
                        ...
                        return (
                          <div 
                            ...
                            className={cn(
                              \"cursor-pointer flex items-center gap-4 p-5 border transition-all duration-300\",
                              isSelected ? \"bg-white/10 border-white/40\" : \"bg-white/5 border-white/5 hover:border-white/20\"
                            )}
                          >
                            <Checkbox checked={isSelected} className=\"border-white/20 data-[state=checked]:bg-bronze data-[state=checked]:border-bronze\" />
                            <span className=\"text-[10px] uppercase tracking-widest font-bold\">{option}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  {currentQuestion?.type === 'text' && (
                    <div className=\"max-w-2xl\">
                      <Textarea 
                        autoFocus
                        placeholder={currentQuestion.placeholder || 'Sua resposta aqui...'}
                        value={answers[currentQuestion.id] || ''}
                        onChange={(e) => handleAnswerChange(currentQuestion.id, e.target.value)}
                        className=\"bg-transparent border-0 border-b border-white/20 rounded-none text-xl p-0 h-24 focus:ring-0 focus:border-bronze transition-all placeholder:text-white/10 font-['Georgia'] italic resize-none\"
                      />
                    </div>
                  )}
                </div>
                ...
                <div className=\"flex items-center gap-6 pt-8\">
                  {step >= 0 && (
                    <button 
                      onClick={handleBack}
                      className=\"text-white/30 hover:text-white flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] transition-all\"
                    >
                  ...
                  {currentQuestion?.type !== 'choice' && currentQuestion?.type !== 'investment' && (
                    <Button 
                      ...
                      className=\"bg-white text-black hover:bg-white/90 rounded-none h-14 px-10 text-[10px] font-bold tracking-[0.3em] uppercase group\"
                    >
                      {step === questions.length - 1 ? (isSubmitting ? 'Enviando...' : 'Finalizar') : 'Próximo'}
      ...
      <footer className=\"relative z-10 p-8 flex justify-between items-end\">
        <div className=\"hidden md:block\">
          <p className=\"text-white/10 text-[9px] uppercase tracking-[0.4em] leading-loose\">
            © 2026 NL ARQUITETOS<br />SÃO JOSÉ DOS CAMPOS · SP
          </p>
        </div>
        <div className=\"text-right\">
          <p className=\"text-white/20 text-[9px] uppercase tracking-[0.4em]\">A ARQUITETURA COMO DECISÃO.</p>
        </div>
      </footer>
