...
const BriefingModal = ({ isOpen, onClose, briefing, onAprovar, onArquivar }: BriefingModalProps) => {
  ...
  const renderProjectInfo = () => {
    ...
    if (tipo === 'arq') {
      content = (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Lote Definido</p>
            <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.imovel_definido || '—'}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Área Terreno</p>
            <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.area_terreno ? `${r.area_terreno} m²` : '—'}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Área Construída</p>
      ...
      content = (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Tipo de Imóvel</p>
            <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.tipo_imovel || '—'}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Área a Reformar</p>
            <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.area_estimada ? `${r.area_estimada} m²` : '—'}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Mobiliário</p>
      ...
      content = (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Tipo de Negócio</p>
              <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.tipo_negocio || '—'}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Perfil do Público</p>
              <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.perfil_cliente || '—'}</p>
            </div>
          </div>
          <div className="space-y-1">
            <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Experiência do Cliente</p>
  ...
  const renderAlignment = () => (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-1">
          <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Estilo</p>
          <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.estilo_referencia || '—'}</p>
        </div>
        <div className="space-y-1">
          <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Orçamento</p>
          <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.orcamento || '—'}</p>
        </div>
        <div className="space-y-1">
          <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Prazo</p>
          <p className="text-xs text-white/80 uppercase tracking-wider font-medium">{r.prazo || '—'}</p>
        </div>
      </div>
      {r.obs && (
        <div className="space-y-1 mt-4">
          <p className="text-[9px] text-white/[0.18] uppercase tracking-widest">Observações</p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl bg-[#0D0D0D] border-[#1E1E1E] text-[#F0EDE8] p-0 gap-0 overflow-hidden rounded-none">
        <DialogHeader className="p-8 border-b border-[#1E1E1E]">
        ...
        <div className="p-8 space-y-10 max-h-[70vh] overflow-y-auto scrollbar-thin scrollbar-thumb-[#1E1E1E] scrollbar-track-transparent">
          {renderContactInfo()}
          {renderProjectInfo()}
          {renderAlignment()}
        </div>

        <div className="p-6 border-t border-[#1E1E1E] bg-[#0D0D0D] flex justify-end gap-3">
          <Button 
            variant="outline"
            onClick={() => {
            ...
            className="border-[#1E1E1E] bg-transparent hover:bg-white/[0.06] text-white/[0.4] rounded-none px-8 h-12 font-['Courier_New'] text-[10px] font-bold uppercase tracking-widest"
          >
          ...
          <Button 
            ...
            className="bg-[#8B7355] hover:bg-[#8B7355]/90 text-[#0D0D0D] rounded-none px-8 h-12 font-['Courier_New'] text-[10px] font-bold uppercase tracking-widest"
          >
