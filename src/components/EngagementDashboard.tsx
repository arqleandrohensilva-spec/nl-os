...
const EngagementDashboard = ({ proposal, onGenerateFollowup }: EngagementDashboardProps) => {
  ...
  if (engagements.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-[#FDFDFD] border border-dashed border-[#E8E4DF] rounded-[2px] text-center animate-fade-in">
        ...
        <p className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] max-w-[320px] leading-relaxed">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white p-4 border border-[#E8E4DF] rounded-[2px] shadow-sm hover:border-bronze/30 transition-all group">
            <p className="text-[9px] uppercase tracking-[0.2em] text-zinc-500 mb-1 font-bold group-hover:text-bronze transition-colors">{card.label}</p>
          ...
          <div className="flex justify-center gap-6 mt-4">
            {deviceData.map((d, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">{d.name}: {d.value}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {[...engagements].reverse().map((eng, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 border border-[#F0EEEB] rounded-[2px] hover:bg-[#FDFDFD] transition-colors">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[8px] text-zinc-500 uppercase">Duração</p>
                </div>
              </div>
            ))}
          </div>
