...
const ForecastModalContent = ({ forecast, navigate }: ForecastModalContentProps) => {
  return (
    <div className="bg-black/90 p-8 overflow-y-auto max-h-[80vh] scrollbar-hide border border-white/[0.06]">
      {/* Totalizador */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-10 border-b border-white/[0.06]">
        <div>
          <span className="text-[10px] font-bold tracking-widest text-white/[0.4] uppercase mb-2 block">FORECAST DO MÊS</span>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">R$ {(forecast.probableTotal / 1000000).toFixed(1)}M</span>
            <span className="text-white/[0.4] text-xs">provável</span>
            <span className="text-white/[0.18] mx-2">/</span>
            <span className="text-white/[0.4] text-xs">R$ {(forecast.totalInGame / 1000000).toFixed(1)}M total em jogo</span>
          </div>
        </div>
        <div className="text-right">
          <div className="text-xs text-white/[0.4] mb-1 font-mono">Meta: R$ {(forecast.meta / 1000000).toFixed(1)}M</div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            {forecast.items.filter(l => l.probabilidade >= 60).length === 0 ? (
              <p className="text-white/[0.12] text-[10px] italic">Nenhum lead nesta faixa.</p>
            ) : (
              forecast.items.filter(l => l.probabilidade >= 60).map(lead => (
                <div 
                  key={lead.id} 
                  onClick={() => navigate('/pipeline')}
                  className="p-4 bg-white/[0.02] border border-white/[0.06] border-l-2 border-l-[#2E7D32] hover:bg-white/[0.04] cursor-pointer transition-all"
                >
                  <div className="text-sm font-medium text-white mb-1">{lead.nome} · {lead.tipo}</div>
                  <div className="text-[10px] text-white/[0.4] leading-relaxed">
            ...
            {forecast.items.filter(l => l.probabilidade >= 30 && l.probabilidade < 60).length === 0 ? (
              ...
              forecast.items.filter(l => l.probabilidade >= 30 && l.probabilidade < 60).map(lead => (
                <div 
                  key={lead.id} 
                  onClick={() => navigate('/pipeline')}
                  className="p-4 bg-white/[0.02] border border-white/[0.06] border-l-2 border-l-[#F59E0B] hover:bg-white/[0.04] cursor-pointer transition-all"
                >
                  <div className="text-sm font-medium text-white mb-1">⚠ {lead.nome} · {lead.tipo}</div>
                  <div className="text-[10px] text-white/[0.4] leading-relaxed">
            ...
            {forecast.items.filter(l => l.probabilidade < 30).length === 0 ? (
              ...
              forecast.items.filter(l => l.probabilidade < 30).map(lead => (
                <div 
                  key={lead.id} 
                  onClick={() => navigate('/pipeline')}
                  className="p-4 bg-white/[0.02] border border-white/[0.06] border-l-2 border-l-[#7A4A3A] hover:bg-white/[0.04] cursor-pointer transition-all"
                >
                  <div className="text-sm font-medium text-white mb-1">✕ {lead.nome} · {lead.tipo}</div>
                  <div className="text-[10px] text-white/[0.4] leading-relaxed">
